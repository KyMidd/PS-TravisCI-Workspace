...
terraform {
  ...
  backend "s3" {
    bucket         = "kyler-globally-unique-bucket"
...
module "bootstrap" {
  ...
  name_of_s3_bucket           = "kyler-globally-unique-bucket"
...
locals {
  ...
  my_public_ip          = "75.166.191.56/32"   # Update this to your public IP when deploying
